...
use crate::wasm::text::{
    RawTextAlign, RawTextDecoration, RawTextDirection, RawTextTransform, helpers as text_helpers,
};
    ...
    with_state_mut!(state, {
        ...
        for fill in &styles.fills {
            if let Ok(raw_fill) = RawFillData::try_from(fill) {
                fill_bytes.extend_from_slice(&<[u8; std::mem::size_of::<RawFillData>()]>::from(raw_fill));
                fill_count += 1;
            }
        }
